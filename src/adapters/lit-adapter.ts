...
function isLitHtmlTemplate(node: Rule.Node): boolean {
  ...
  const te = node as unknown as {
    tag: { type: string; name?: string; property?: { name?: string } };
...
function parseAttributeName(raw: string): {
  ...
  if (raw.startsWith('.')) return { name: raw.slice(1), bindingType: '.' };
  if (raw.startsWith('?')) return { name: raw.slice(1), bindingType: '?' };
  if (raw.startsWith('@')) return { name: raw.slice(1), bindingType: '@' };
  return { name: raw };
}
...
function extractFromParse5(
  ...
  const nodes = (fragment as unknown as { childNodes: Parse5Node[] }).childNodes;
...
export function extractElementsFromTemplate(node: Rule.Node): ParsedElement[] {
  if (!isLitHtmlTemplate(node)) return [];
