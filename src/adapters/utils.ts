...
export function isSwcJsxTag(name: string): boolean {
  if (name.startsWith('sp-') || name === 'overlay-trigger') return true;
  if (/^Sp[A-Z]/.test(name)) return true;
  if (name === 'OverlayTrigger') return true;
  return false;
}
...
export function resolveJsxTagName(name: string): string {
  if (name.startsWith('sp-') || name === 'overlay-trigger') return name;
  if (name === 'OverlayTrigger') return 'overlay-trigger';
